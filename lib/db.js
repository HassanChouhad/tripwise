...
export function getHotelDetailsFromDb(hotelId) {
  const db = getDb();
  return db.prepare('SELECT * FROM hotels WHERE id = ?').get(hotelId);
}

export function getFlightById(flightId) {
  const db = getDb();
  return db.prepare('SELECT * FROM flights WHERE id = ?').get(flightId);
}
